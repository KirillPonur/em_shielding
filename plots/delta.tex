...
\pgfplotstableset{
	...
	create on use/deltal/.style={
		\xdef\speed{3*10^10}
	    create col/expr={
	    	12.328/sqrt(\thisrow{f})
	    }
	},	
}
	\begin{tikzpicture}
	  ...
	  %          {$\pgfmathprintnumber[frac, frac denom=32,frac whole=false]{\pgfmathresult}\pi$}
	    ...
	    \addplot [samples=100, domain=20:10000, dashed, line width=1.2pt] {12.328/sqrt(x)};
	    \addlegendentry{$\delta(f)$, латунь }

	    % \addplot [samples=100, domain=20:10000, line width=1.2pt] {0.807/sqrt(x)};
	    % \addlegendentry{$\delta(f)$, сталь [см]}

	    \addplot [samples=100, domain=20:10000, blue, line width=1.2pt] {x/x*1}; 	    \addlegendentry{10 мм}
	    \addplot [samples=100, domain=20:10000, red, line width=1.2pt] {x/x*0.5}; 	    \addlegendentry{5 мм}
	    \addplot [samples=100, domain=20:10000, black, line width=1.2pt] {x/x*0.2}; 	    \addlegendentry{2 мм}
	     % \xdef\C{5e-8}
	     % \xdef\R{13000}
% 		\addplot +[mark=*, line width=1.2pt,black] plot [error bars/.cd, y dir = both, y explicit] table [ x=f, y=eta, ] {../data/data.tsv};
% 			\addlegendentry{Латунь 2}
% 		\addplot +[mark=*, line width=1.2pt,red] plot [error bars/.cd, y dir = both, y explicit] table [ x=f, y=eta5, ] {../data/data.tsv};
% 			\addlegendentry{Латунь 5}
% 		\addplot +[mark=*, line width=1.2pt,blue] plot [error bars/.cd, y dir = both, y explicit] table [ x=f, y=eta10, ] {../data/data.tsv};
