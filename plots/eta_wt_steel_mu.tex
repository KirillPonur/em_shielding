...
\begin{tikzpicture}[
  declare function={
  		Mu(\f)=153;
  		% Mu(\f)=200*(1-1/(1+exp(-2*(\f-1000)*10/10000)));
		Delta(\f)=18.04/sqrt(\f)/sqrt(Mu(\f));
		X(\f)=Mu(\f)/2.861*Delta(\f);
		eta(\f,\d,\tu)=1/6*exp(\d/Delta(\f))*sqrt(9+2/X(\x)^2+6/X(\x)+6*X(\x)+2*X(\x)^2)*1;
		Eta(\f,\d,\tu)=sqrt(1+(2/3*2.861*\d/Delta(\f)^2)^2);
  }
]	
		    \begin{axis}[
			grid=both,
